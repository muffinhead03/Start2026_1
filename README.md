...
## 🤖 AI 힌트 시스템
-   플레이어의 체류 시간, 힌트 요청 횟수, 실패 횟수, 진행률, 반복 조사 횟수 및 요청 방식을 분석해 HintEngine이 hintLevel 1~5를 결정
- Ollama(gemma3:4b) 로컬 LLM이 씬 컨텍스트 기반 한국어 힌트 1~2문장 생성
- 씬당 2회 사용 제한, 외부 서버 없이 완전 로컬 실행
- AI 응답이 일정 시간(10초) 내 도착하지 않을 경우, 현재 힌트 레벨에 맞는 Fallback힌트를 반환한다. 

## 🎮 게임 시스템
- **상자 개봉 이벤트**: 금지된 상자를 여는 순간 공포 이벤트 트리거, 이후 모든 씬에 이상현상 발생
- **단계형 공포 방탈출 구조**: 6개의 각 스테이지마다 고유한 퍼즐 구조 (와인잔 얼룩 교차점, 오르간 수리 및 연주 등)
...
힌트 수준 결정은 LLM에 맡기지 않고 HintEngine의 결정론적 가중 점수 알고리즘이 담당합니다. LLM은 결정된 힌트 수준과 씬 컨텍스트를 바탕으로 자연스러운 문장을 생성합니다. 즉, “판단은 코드가, 문장은 AI가”라는 원칙으로 예측 가능한 힌트 제어와 표현 다양성을 함께 확보했습니다.
...
| Gemini | README 초안, 기본 이동 스크립트, Unity 구현 탐색 | ✅ 실제 상황과 맞는지 검토, 구현 구조가 팀의 의도랑 맞는지 확인 |
...
> **현재 공개 빌드 기준:** Room 1과 Room 2를 플레이할 수 있으며, 로컬 AI 힌트 시스템은 두 Room에 연동되어 있습니다.

| 항목          | 상태      | 설명                                                                        |
| ----------- | ------- | ------------------------------------------------------------------------- |
| Room 1      | 구현 완료   | 와인 얼룩 색상 → 알파벳 시프트 → 가구 이름 해독 퍼즐                                          |
| Room 2      | 구현 완료   | 파이프 수집 → 오르간 수리 → 악보 연주 퍼즐                                                |
| AI 힌트 시스템   | 구현 완료   | HintEngine이 플레이어 상태를 바탕으로 `hintLevel` 1~5를 결정하고, Ollama가 자연어 힌트를 생성       |
| 폴백 힌트       | 구현 완료   | AI 응답이 10초를 초과하면 힌트 레벨에 맞는 고정 힌트를 반환                                      |
| 요청 방식 선택 UI | 미구현     | 현재 `hintType`은 `indirect`로 고정되어 동작하며, `direct / indirect` 선택 UI는 후속 구현 예정 |
| Room 3~6    | 개발 진행 중 | 피아노 룸, 인형 방, 거울 방, 양초 방의 퍼즐·엔티티·힌트 컨텍스트를 후속 구현 예정                         |
